/**
 * Parallax scroll background from parallax-scroll-animation (index.html + style.css + index.js).
 * Loads SVG, then runs GSAP ScrollTrigger: scene1 (hills, sun, clouds, bird) → scene2 (hills2, bats) → scene3 (night, stars).
 */
...
export async function initParallaxScroll() {
  const container = document.getElementById("parallax-svg-container");
  const scrollEl = document.querySelector(".scrollElement");
  if (!container || !scrollEl) return;

  try {
    const res = await fetch("/parallax-svg.html");
    const html = await res.text();
    container.innerHTML = html.trim();
  } catch (e) {
    console.warn("Parallax SVG failed to load", e);
    return;
  }

  if (!document.getElementById("parallax-svg")) return;
